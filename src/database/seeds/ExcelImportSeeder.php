...
class ExcelSeeder extends Seeder {
	...
	public function run() {
		...
		// $bar = $this->command->getOutput()->createProgressBar(count($sheets));
		// $bar->start();
		// $bar->setFormat(
		// 	"\n%message%
		//               %current%/%max% [%bar%] %percent:3s%%\n"
		// );
		foreach ($sheets as $sheet_name => $sheet_detail) {
			$this->command->getOutput()->newLine(1);
			$this->command->info($sheet_name . ' STARTED');
			// $bar->setMessage($sheet_name . ' STARTED', 'message');
			...
			Excel::selectSheets($sheet_name)->load($excel_file_path, function ($reader) use ($sheet_name, $sheet_detail, $company, $specific_company, $tc, $command) {
				...
				$reader->limitRows($sheet_detail['row_limit']);
				$records = $reader->get();
